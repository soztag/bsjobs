...
r_coms <- imap_chr(.x = py_objs$res, .f = function(x, y) {
  if (inherits(x = x, what = "praw.models.reddit.more.MoreComments")) {
    NA
  } else {
   x$body 
  }
})
# remove NAs which are actually just "more" tags
r_coms <- r_coms[!is.na(r_coms)]
readr::write_rds(x = r_coms, path = "r_coms.rds")
```
